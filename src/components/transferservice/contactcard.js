...
const Contactcard = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const apiUrl = "https://4ac6-2400-adc5-463-9d00-9c13-5215-5a37-b36e.ngrok-free.app/rent-luxury/custom-cars";  // Replace with your actual API endpoint
    ...
    try {
      ...
      if (response.status === 201) {
        ...
        Swal.fire({
          icon: 'success',
          title: language === 'en' ? 'Success!' : language === 'it' ? 'Successo!' : language === 'du' ? 'Succes!' : language === 'fr' ? 'Succès!' : 'Success!',
          text: language === 'en' 
        ...
        Swal.fire({
          icon: 'error',
          title: language === 'en' ? 'Error!' : language === 'it' ? 'Errore!' : language === 'du' ? 'Fout!' : language === 'fr' ? 'Erreur!' : 'Error!',
          text: language === 'en' 
      ...
      <div className="contact_card1">
        ...
        <div className="contact_card1_form1" style={{ width: "100%" }}>
          <div className="contact_div">
            <Form.Label className="contact_card1_form1_label1">{language === 'en' ? 'Name' : language === 'it' ? 'Nome' : language === 'du' ? 'Naam' : language === 'fr' ? 'Nom' : 'Name'}</Form.Label>
            <Form.Control
              type="text"
              name="name"
              value={formData.name}
          ...
          <div className="contact_div">
            <Form.Label className="contact_card1_form1_label1">{language === 'en' ? 'Email' : language === 'it' ? 'Email' : language === 'du' ? 'E-mail' : language === 'fr' ? 'E-mail' : 'Email'}</Form.Label>
            <Form.Control
              type="email"
              name="email"
          ...
          <div className="contact_div">
            <Form.Label className="contact_card1_form1_label1">{language === 'en' ? 'Phone Number' : language === 'it' ? 'Numero di telefono' : language === 'du' ? 'Telefoonnummer' : language === 'fr' ? 'Numéro de téléphone' : 'Phone Number'}</Form.Label>
            <Form.Control
              type="tel"
          ...
          <div className="contact_div">
            <Form.Label className="contact_card1_form1_label1">{language === 'en' ? 'Description' : language === 'it' ? 'Descrizione' : language === 'du' ? 'Beschrijving' : language === 'fr' ? 'Description' : 'Description'}</Form.Label>
            <Form.Control
              type="text"
              name="description"
        ...
        <div className="contact_div1">
          ...
          <button onClick={handleSubmit}>
            {language === 'en' ? 'Request Callback' : language === 'it' ? 'Richiedi Richiamata' : language === 'du' ? 'Vraag om Terugbellen' : language === 'fr' ? 'Demander un rappel' : 'Request Callback'}
          </button>
        </div>
      </div>
    </div>
